...
# calculate slope
yout = [(1.5*y[i+1]-2.0*y[i]+0.5*y[i])/(dt) for i in range(1,len(y)-1)]
...
#xtrain = [[y[i-2,:], y[i-1,:]] for i in range(2,y.shape[0])]
...
for i in range(nsamples):
    ...
    # calculate slope
    yout = [(1.5*y[i+1]-2.0*y[i]+0.5*y[i])/(dt) for j in range(1,len(y)-1)]
    yout = np.array(yout)
...
history_callback = custom_model.fit(xtrain, ytrain, epochs=100, batch_size=50, verbose=1, validation_split= 0.3,
...
for k in range(nsamples):
    ...
    ytest_ml = np.vstack((ytest_ml, ytode[1]))
    
    for i in range(2,nt_steps):
        slope_ml = custom_model.predict(ytest) # slope from LSTM/ ML model
        d = (4.0/3.0)*ytest_ml[i-1]- (1.0/3.0)*ytest_ml[i-2] + (2.0/3.0)*dt*slope_ml[0] # y1 at next time step
        ytest_ml = np.vstack((ytest_ml, d))
        e = d.reshape(1,3)
        ytemp = ytest[0]
        ytemp = ytemp.reshape(1,6)
        ee = np.concatenate((ytemp,e), axis = 1)
        ee = ee[0,3:]
        ytest = ee.reshape(1,6)
