...
# calculate slope
yout = [(y[i+1]-y[i-1])/(2.0*dt) for i in range(3,len(y)-1)]
...
for i in range(nsamples):
    ...
    # calculate slope
    yout = [(y[j+1]-y[j-1])/(2.0*dt) for j in range(3,len(y)-1)]
    yout = np.array(yout)
    # temporary storage
...
for k in range(nsamples):
    ...
    for i in range(4,nt_steps):
        ...
        a = ytest_ml[i-2,0] + 2.0*dt*slope_ml[i-4,0] # y1 at next time step
        b = ytest_ml[i-2,1] + 2.0*dt*slope_ml[i-4,1] # y2 at next time step
        c = ytest_ml[i-2,2] + 2.0*dt*slope_ml[i-4,2] # y3 at next time step
    ...
    plt.xlabel('time ML')
    plt.legend(loc='best')
    plt.savefig('ode_leapfrog_fourleg_x=-0.25.png', dpi = 1000)
    plt.show()
